...
bool BSPFile::parse(const std::string& bsp_directory, const std::string& bsp_file)
{
    ...
    try
    {
        ...
        {
            std::cout << "BSPFile::parse(): " << bsp_file << "has an unknown BSP version, trying to parse it anyway..."
                << std::endl;
        }
...
bool BSPFile::parse_leaffaces(std::ifstream& bsp_binary)
{
    try
    {
        ...
        {
            std::cout << "BSPFile::parse_leaffaces(): map has to many leaffaces, parsed more than required.." <<
                std::endl;
        }
...
bool BSPFile::parse_leafbrushes(std::ifstream& bsp_binary)
{
    try
    {
        ...
        if (num_leaffaces > MAX_MAP_LEAFBRUSHES)
        {
            std::cout << "BSPFile::parse_leaffaces(): map has to many leafbrushes, parsed more than required.." <<
                std::endl;
        }
        else if (!num_leaffaces)
        {
