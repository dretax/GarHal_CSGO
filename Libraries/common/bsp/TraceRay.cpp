...
void TraceRay::ray_cast_node(BSPFile* pBSPFile, const int32_t node_index, const float start_fraction,
                             const float end_fraction, const Vector3& origin, const Vector3& final, trace_t* pTrace)
{
    ...
    if (node_index < 0)
    {
        ...
        {
            ray_cast_surface(pBSPFile, static_cast<int32_t>(pBSPFile->m_Leaffaces.at(pLeaf->m_Firstleafface + i)),
                             pTrace, origin, final);
        }
...
void TraceRay::ray_cast_brush(BSPFile* pBSPFile, dbrush_t* pBrush, trace_t* pTrace, const Vector3& origin,
                              const Vector3& final)
{
...
void TraceRay::ray_cast_surface(BSPFile* pBSPFile, const int32_t surface_index, trace_t* pTrace, const Vector3& origin,
                                const Vector3& final)
{
    ...
    if (dot1 > 0.f != dot2 > 0.f)
    {
        ...
        for (i = 0; i < pPolygon->m_nVerts; ++i)
        {
            ...
            {
                pEdgePlane->m_Origin = pPlane->m_Origin - (pPolygon->m_Verts.at(i) - pPolygon->m_Verts.at(
                    (i + 1) % pPolygon->m_nVerts));
